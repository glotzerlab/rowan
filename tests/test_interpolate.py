...
class TestInterpolate(unittest.TestCase):
    ...
    def test_slerp(self):
        ...
        self.assertTrue(
                np.allclose(
                    interpolate.slerp(one, root_two, 0),
                    ans[0, :]
                    )
                )
        self.assertTrue(
                np.allclose(
                    interpolate.slerp(one, root_two, 1),
                    ans[1, :]
                    )
                )
        self.assertTrue(
                np.allclose(
                    interpolate.slerp(one, root_two, 0.5),
                    ans[2, :]
                    )
                )
        self.assertTrue(
                np.allclose(
                    interpolate.slerp(one, root_two, [0, 1, 0.5]),
                    ans
                    )
                )
        ...
        self.assertTrue(np.all(
            interpolate.slerp(test1, test1, 0) ==
            test1))
        self.assertTrue(np.allclose(
            interpolate.slerp(test1, test1, 1),
            test1))
        self.assertTrue(np.all(
            interpolate.slerp(test1, test1, 0.5) ==
            test1))
        self.assertTrue(
                np.allclose(
                    interpolate.slerp(
    ...
    def test_squad(self):
        ...
        self.assertTrue(
                np.allclose(
                    interpolate.squad(one, one, one, root_two, 1),
                    root_two
                    )
                )
        self.assertTrue(
                np.allclose(
                    ...
                    np.array([0.98078528, 0.19509032, 0, 0.])
                    )
                )
