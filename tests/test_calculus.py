...
class TestCalculus(unittest.TestCase):
    ...
    def test_derivative(self):
        ...
        self.assertTrue(np.all(
            calculus.derivative(one, one_vec) == [0, 0.5, 0, 0]))

        x = np.array([np.sqrt(2)/2, np.sqrt(2)/2, 0, 0])
        v = np.array([0.1, 0.1, 0.1])
        ans = np.array([-0.035355339059327383,
                        0.035355339059327383,
                        0.0,
                        0.070710678118654766])

        self.assertTrue(
                np.allclose(
                    calculus.derivative(x, v),
                    ans
                    )
                )
    ...
    def test_integrate(self):
        ...
        ans = np.array([0.87758256, 0.47942554, 0., 0.])
        self.assertTrue(np.allclose(
            calculus.integrate(one, one_vec, 1), ans))
