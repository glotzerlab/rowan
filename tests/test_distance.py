...
class TestDistance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.saved_data = np.load(os.path.join(
            os.path.dirname(__file__),
            'files/test_geometry.npz'))
    ...
    def test_riemann_exp_map(self):
        ...
        self.assertTrue(
                np.allclose(
                    geometry.riemann_exp_map(one, one), [np.exp(1), 0, 0, 0]
                    )
                )
    ...
    def test_intrinsic_distance(self):
        ...
        self.assertTrue(
                np.allclose(
                    geometry.intrinsic_distance(one, one), 0
                    )
                )
    ...
    def test_sym_intrinsic_distance(self):
        ...
        self.assertTrue(
                np.allclose(
                    geometry.sym_intrinsic_distance(one, one), 0
                    )
                )
